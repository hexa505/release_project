...
public interface BookmarkRepository extends JpaRepository<Bookmark, Long> {
    ...
    @Query("select b, concat(function('lpad', album.modifiedDate, 10), function('lpad', b.id, 10, '0')) from Bookmark b "
            + "join fetch b.album album "
            + "join fetch album.user writer "
            + "where b.user.id = :userId and "
            + "concat(function('lpad', album.modifiedDate, 10), function('lpad', b.id, 10, '0')) "
            + "< concat(function('lpad', :dateTime, 10), function('lpad', :bookmarkId, 10, '0'))"
            + "order by album.modifiedDate desc, b.id desc")
